...
@SpringBootTest(classes = CodeupSpringBlogExercisesApplication.class)
...
public class PostsIntegrationTests {
    ...
    public void testCreatePost() throws Exception {
        ...
        this.mvc.perform(
                post("/posts/makeapost").with(csrf())
                        .session((MockHttpSession) httpSession)
                        // Add all the required parameters to your request like this
                        .param("title", "test")
                        .param("body", "testy post"))
...
//                .session((MockHttpSession) httpSession)
    ...
    public void testEditPost() throws Exception {
        ...
        // Makes a Post request to /posts/makeapost and expect a redirection to the Ad show page
        this.mvc.perform(
                post("/posts/makeapost").with(csrf())
                        .session((MockHttpSession) httpSession)
                        .param("editID", String.valueOf((int) existingPost.getId()))
                        .param("title", "edited title")
                        .param("body", "edited body"))
                .andExpect(status().is3xxRedirection());

        // Makes a GET request to /posts/{id} and expect a redirection to the Ad show page
        this.mvc.perform(get("/posts/" + (int) existingPost.getId()))
                .andExpect(status().isOk())
                // Test the dynamic content of the page
